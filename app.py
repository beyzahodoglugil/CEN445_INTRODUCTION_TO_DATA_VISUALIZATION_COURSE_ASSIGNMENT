...
def chart_scatter_matrix_sleep_exercise_stress(df: pd.DataFrame):
    st.subheader("😴 Sleep, Exercise & Stress Relationship (Scatter Matrix)")
    st.write("Explore relationships between Sleep Hours, Screen Time and Happiness. Color = Stress Level, Symbol = Exercise Level.")

    if df.empty:
        st.warning("No data available for selected filters.")
        return

    numeric_cols = ["Sleep Hours", "Screen Time per Day (Hours)", "Happiness Score"]
    ...
    fig = px.scatter_matrix(
        ...
        color="Stress Level" if "Stress Level" in df.columns else None,
        symbol="Exercise Level" if "Exercise Level" in df.columns else None,
        opacity=0.45,
        hover_data=["Age", "Country", "Gender"] if set(["Age", "Country", "Gender"]).issubset(df.columns) else None,
        color_discrete_map=color_map if "Stress Level" in df.columns else None,
    )
...
def main():
    ...
    st.markdown("**CEN445 Data Visualizations - Nadire Şeker **")
